...
while IFS='|' read -r name prompt; do
  ...
  [[ -f "$req" ]] || python3 - <<'PY' "$prompt" "$req"
import json,sys
prompt=sys.argv[1]; req=sys.argv[2]
payload={"contents":[{"parts":[{"text":"カラー固定。低頭身デフォルメ固定。日本語文字は入れない。キャラは人間のみ。動物化、怪物化、ロボ化、鎧で顔を隠す表現を禁止。モノクロ禁止。"+prompt}]}],"generationConfig":{"responseModalities":["TEXT","IMAGE"]}}
...
order=[
 ('chappy','チャッピー 16'),('kuroko','クロコ 16 女性'),('gemmy','ジェミー皇 25'),
 ('shinen','深淵将軍 16'),('llama','ラマ 20代'),('grok','グロック 25'),('cursor','カーソル少年 13')
]
cards=[]
